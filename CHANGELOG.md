...
- Lock the TinyStories recurrent/input consolidation window at `learning.plasticity_window = 8` after the P100 1024-story A/B: W8 completed the train+validation run in `568.38 s`, held-out quality was `3.32556 bits/byte`, and the clean canonical 16-story benchmark reached `3843.52 steps/s`; W8 dominated W4 on both speed and held-out bits/byte and beat W16 on full-run wall time and held-out bits/byte.
- Add Formula-v3 surprise-gated recurrent/input plasticity with `learning.plasticity_confidence_threshold`. TinyStories uses `0.50`: an already-scheduled W8 consolidation runs only while the target probability is below 50% (end-document always consolidates). Eligibility traces, direct output/context supervision, inhibitory homeostasis, and replay target selection remain unchanged. Test/probe configs use `1.00` to preserve the existing exact/conformance path.
