...
//! runtime; the stable v1 line preserves the clean artifact baseline established by v1.0.0.
...
fn decode_model(bytes: &[u8]) -> LeoResult<Model> {
    ...
    if contract != SemanticsContract::CURRENT {
        return Err(LeoError::checkpoint_incompatible(format!(
            "checkpoint semantics {:?} do not match the current Leo v1 semantics {:?}",
            contract,
...
fn validate_header(header: &[u8]) -> LeoResult<()> {
    ...
    if &header[0..8] != MAGIC {
        return Err(LeoError::checkpoint_incompatible(
            "unsupported checkpoint magic; the current Leo v1 runtime accepts only PSCLS100 artifacts",
        ));
    }
