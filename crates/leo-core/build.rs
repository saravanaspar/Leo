...
fn main() {
    ...
    rust.push_str(&format!(
        "const _: [(); 1] = [(); (crate::semantics::CUDA_ABI_VERSION == {abi_version}) as usize];\n\n"
    ));
    rust.push_str("#[repr(C)]\n#[derive(Clone, Copy)]\nstruct CudaConfig {\n");
    for (ty, name) in &config {
        rust.push_str(&format!("    {name}: {ty},\n"));
    }
    rust.push_str("}\n\n");
    rust.push_str(
        "#[repr(usize)]\n#[derive(Clone, Copy, Debug, PartialEq, Eq)]\nenum PersistentPointer {\n",
    );
    for (i, name) in persistent.iter().enumerate() {
        rust.push_str(&format!("    {} = {i},\n", rust_variant(name)));
    }
    rust.push_str(
        "}\nimpl PersistentPointer { const fn index(self) -> usize { self as usize } }\n",
    );
    rust.push_str(&format!(
        "const PERSISTENT_POINTER_COUNT: usize = {};\n\n",
        persistent.len()
    ));
    rust.push_str(
        "#[repr(usize)]\n#[derive(Clone, Copy, Debug, PartialEq, Eq)]\nenum BatchDeltaPointer {\n",
    );
    for (i, name) in delta.iter().enumerate() {
        rust.push_str(&format!("    {} = {i},\n", rust_variant(name)));
    }
    rust.push_str(
    ...
    rust.push_str(&format!(
        "const BATCH_DELTA_POINTER_COUNT: usize = {};\n",
        delta.len()
    ));
    ...
    for (i, name) in persistent.iter().enumerate() {
        header.push_str(&format!("    LEO_P_{name} = {i},\n"));
    }
    ...
    for (i, name) in delta.iter().enumerate() {
        header.push_str(&format!("    LEO_D_{name} = {i},\n"));
    }
    header.push_str(&format!("    LEO_D_POINTER_COUNT = {}\n}};\n", delta.len()));
    header.push_str(&format!(
        "static_assert(sizeof(LeoConfig) == {}, \"LeoConfig ABI size mismatch\");\n",
        config.len() * 4
    ));
